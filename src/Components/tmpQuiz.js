...
const Quiz = (props) => {
        ...
        userService.getQuiz(id).then((response) => {
            if (response.data.status === 200) {
                ...
                for (let i = 0; i < data.questions.length; i++) {
                    tmpArry.push({ id: data.questions[i]._id, answer: 0 });
                    tmpArry2.push(false);
                }
                ...
                setTimeout(() => {
                    let time = new Date();
                    setMaxQuizTime(10);
                    setQuizTime(time.setSeconds(time.getSeconds() + 10));
                }, 300);
            } else {
                setIsLoading(false);
                setQuizData({});
    ...
    function handleAnswerOptions(answerId, questionId, index, isRight) {
        ...
        let tmp2 = answerList;
        tmp.questions[index] = { id: questionId, answer: answerId };
        tmp2[index] = isRight;
    ...
    function Steps(props) {
        ...
        if (!props.questionInfo.answers[0].value.startsWith('/images/')) {
            return (
                <div className="qstns_box">
